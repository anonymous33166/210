...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Run legal dataset experiments')
...
def load_legal_config():
    """Load optional legal custom config; fallback to empty when missing.

    To simplify packaging for reviewers, we avoid a hard dependency on
    external JSON config files. If the file exists, we load it; otherwise
    return an empty dict so the system runs with built-in defaults.
    """
    try:
        config_path = project_root / "config/legal_config.json"
...
def validate_legal_setup():
    print("🔍 Validating legal experiment setup...")
    dataset_path = project_root / "data/legal_dataset.json"
    if not dataset_path.exists():
        raise FileNotFoundError(f"Legal dataset file not found: {dataset_path}")
    print("  ✅ Legal dataset exists")
    print("  ✅ Legal experiment setup validated")
    return True


def create_legal_experiment_config(args):
    custom_cfg = load_legal_config()
    config = ExperimentConfig(
        datasets=["legal"],
        methods=args.methods,
        runs_per_experiment=args.runs,
        max_nodes_per_experiment=40,
        parallel_experiments=args.parallel,
        output_dir=f"results/legal_experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
...
def main():
    ...
    print("🚀 Starting legal dataset experiments")
    print("=" * 60)
    ...
    config = create_legal_experiment_config(args)
    ...
    print("\n🎯 Starting legal dataset experiments...")
    start_time = time.time()

    try:
        experiment_system.run_comprehensive_experiments()
        duration = time.time() - start_time
        print(f"\n✅ Legal dataset experiments completed! ⏱️ {duration:.2f} s")
