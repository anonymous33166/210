...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Run Gaokao math dataset experiments')
    parser.add_argument('--methods', nargs='+', default=['cot','tot','got','aot','egot'], help='Specify reasoning methods')
...
def load_gaokao_config():
   
    try:
        config_path = project_root / "config/gaokao_config.json"
...
def validate_gaokao_setup():
    print("🔍 Validating Gaokao math experiment setup...")
    dataset_path = project_root / "data/gaokao_math_dataset.json"
    if not dataset_path.exists():
        raise FileNotFoundError(f"Gaokao math dataset file not found: {dataset_path}")
    print("  ✅ Gaokao math dataset exists")
    print("  ✅ Gaokao math experiment setup validated")
    return True


def create_gaokao_experiment_config(args):
    custom_cfg = load_gaokao_config()
    config = ExperimentConfig(
        ...
        output_dir=f"results/gaokao_experiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
...
def main():
    ...
    print("🚀 Starting Gaokao math dataset experiments")
    print("=" * 60)
    ...
    config = create_gaokao_experiment_config(args)

    print("\n🏗️ Creating experiment system...")
    experiment_system = EnhancedExperimentSystem(config)

    if args.limit:
        experiment_system.set_dataset_limit(args.limit)
    ...
    print("\n🎯 Starting Gaokao math experiments...")
    start_time = time.time()

    try:
        experiment_system.run_comprehensive_experiments()
        duration = time.time() - start_time
        print(f"\n✅ Gaokao math experiments completed! ⏱️ {duration:.2f} s")
